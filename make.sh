...
if [[ "${DEPS}" == "1" ]]; then
  ...
  if [ "${OS}" == "Linux" ]; then
    ...
    if [[ "${DISTRO}" == "Ubuntu" ]]; then
      sudo apt -y install mp3info python3-pip libtag1-dev libchromaprint-dev ubuntu-restricted-extras libav-tools && pip3 install -U pyacoustid pytaglib || exiterr "deps failed (linux), exiting."
    else
...
if [[ "${DOC}" == "1" ]]; then
  if [[ -x "$(command -v help2man)" ]]; then
    ...
    help2man -n "automatically identify, tag and rename audio files" -N -o idntag.1 ./idntag && ${SED} "s/\.\\\\\" DO NOT MODIFY THIS FILE\!  It was generated by help2man.*/\.\\\\\" DO NOT MODIFY THIS FILE\!  It was generated by help2man./g" idntag.1 || exiterr "doc failed, exiting."
  fi
fi
